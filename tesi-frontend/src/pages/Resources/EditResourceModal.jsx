...
export default function EditResourceModal({ resourceToEdit, onClose, onSave, existingResources = [] }) {
  if (!resourceToEdit) return null;

  const [error, setError] = useState('');

  // 1. Capiamo se è VM o Baremetal per caricare i dropdown giusti
  const isBaremetal = resourceToEdit.siteId && resourceToEdit.siteId.includes('baremetal');
  const resourceType = isBaremetal ? 'baremetal' : 'vm';

  // 2. Parsiamo la durata esistente (es: da "14d" a numero 14 e unità "d")
  const initialDurationStr = resourceToEdit.duration || '3h';
  const initialUnit = initialDurationStr.slice(-1);
  const initialValue = parseInt(initialDurationStr.slice(0, -1), 10) || 3;

  // ==========================================
  // STATI DEL FORM
  // ==========================================
  const [name, setName] = useState(resourceToEdit.name || '');
  const [experiment, setExperiment] = useState(resourceToEdit.experiment || '');
  
  const [diskImage, setDiskImage] = useState(resourceToEdit.diskImage || siteData[resourceType].images[0]);
  const [flavor, setFlavor] = useState(resourceToEdit.flavor || siteData[resourceType].flavors[0].value);
  
  const [durationValue, setDurationValue] = useState(initialValue);
  const [durationUnit, setDurationUnit] = useState(initialUnit);

  const [publicIpv4, setPublicIpv4] = useState(resourceToEdit.publicIpv4 || false);
  const [sshKey, setSshKey] = useState(resourceToEdit.sshKey || '');
  ...
  const handleSubmit = (e) => {
    ...
    // 1. Trasformiamo in modo sicuro l'esperimento in testo (risolve il crash dei numeri)
    const expString = String(experiment || '');
    ...
    // 2. Usiamo la stringa sicura per la validazione
    if (!expString.trim()) return setError('Experiment ID/Name is required.');

    // Controllo doppioni escludendo se stessi
    const isDuplicate = existingResources.some(res => 
      res.id !== resourceToEdit.id && res.name.toLowerCase() === name.trim().toLowerCase()
    );
    if (isDuplicate) return setError('A resource with this name already exists.');

    // Validazione Lifetime
    ...
    const updatedData = {
      ...resourceToEdit,
      name: name.trim(),
      experiment: expString.trim(), // 3. Usiamo la stringa sicura nel salvataggio!
      diskImage,
      flavor,
    ...
    onSave(resourceToEdit.id, updatedData);
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/40 backdrop-blur-sm animate-in fade-in duration-200" onClick={onClose}>
      <div
        className="bg-white rounded-4xl w-full max-w-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]"
      ...
      >
        ...
        <div className="flex items-center justify-between px-8 py-6 border-b border-gray-100 shrink-0">
          <div>
            ...
            <h2 className="text-2xl font-extrabold text-gray-900 tracking-tight line-clamp-1">
              {resourceToEdit.name}
            </h2>
          </div>
        ...
        {/* FOOTER */}
        <div className="px-8 py-5 flex items-center justify-between border-t border-gray-100 bg-white shrink-0">
          
          {/* AREA ERRORE: in basso a sinistra */}
